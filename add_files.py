...
def on_any_event(event):
    conn1 = sqlite3.connect(
        r'C:\Users\akush\Desktop\Programming\Projects\Safe_Sql\safe.sqlite')
    cur = conn1.cursor()
    for filename in os.listdir(path):
        ...
        file = binary_data
        cur.execute('INSERT OR REPLACE INTO Files (name, ext, file) VALUES (?, ?, ?)',
                    (name, ext, file,))
...
if __name__ == '__main__':
    ...
    cur1 = conn.cursor()
    cur1.executescript(
        '''Create table if not exists Files (name TEXT, ext TEXT, file BLOB NOT NULL UNIQUE)''')
    cur1.close()
